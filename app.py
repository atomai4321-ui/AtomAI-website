...
DPO_CSV_PATH = DATA_DIR / "dpo_database.csv"
...
# Load & clean data
#
# MEMORY NOTES (learned the hard way on a 512MB free host):
#
# 1. `dpo_index` used to store {key: [full row dict, ...]} — a SECOND
#    complete copy of the entire database as Python dict objects. Fixed:
#    it now stores lightweight integer row positions and rows are pulled
#    from dpo_df (the single source of truth) on demand via .iloc[].
#
# 2. print() is buffered when Render captures it (not a real terminal), so
#    if the process gets SIGKILL'd mid-load, buffered lines never reach
#    the log. Every checkpoint below is flush=True so the next attempt
#    shows exactly which step used the memory, even if it dies.
# ----------------------------------------------------------------------
...
except ImportError:  # resource is Unix-only; harmless no-op elsewhere
...
def _log(msg: str) -> None:
    print(f"[AtomAI] {msg} (peak {_peak_mb():,.0f} MB)", flush=True)


_log("Loading databases...")

if DPO_PARQUET_PATH.exists():
    # Fast path: Parquet is a compressed, column-oriented binary format —
    # dramatically faster and lower-memory to load than parsing an 80MB+
    # CSV through pandas' text parser on every deploy/restart. Build this
    # file locally with convert_to_parquet.py and commit it; it's what
    # makes startup reliable on a memory-constrained free host.
    _log(f"Loading {DPO_PARQUET_PATH.name} (fast path)...")
    dpo_df = pd.read_parquet(DPO_PARQUET_PATH)
    dpo_df.columns = dpo_df.columns.str.strip()
    _log("Parquet read into memory")
else:
    ...
    _log(f"No Parquet file found — parsing {DPO_CSV_PATH.name} directly...")
    dpo_df = pd.read_csv(DPO_CSV_PATH, dtype=str, keep_default_na=False, low_memory=False)
    dpo_df.columns = dpo_df.columns.str.strip()
    for col in dpo_df.columns:
        dpo_df[col] = dpo_df[col].str.strip()
    # Columns with a small number of repeated values compress dramatically
    # with category dtype (pandas stores each unique string once + an
    # integer code per row, instead of a full string per row).
    for col in ("DPO_Type", "A_site_key", "B_site_key", "Material_Status"):
        if col in dpo_df.columns:
            dpo_df[col] = dpo_df[col].astype("category")
    _log("CSV read into memory")

# Normalize "stable" into a real boolean column regardless of whether it
# arrived as a Parquet bool or as CSV text ("1" / "True" / etc).
if dpo_df["stable"].dtype != bool:
    dpo_df["stable"] = dpo_df["stable"].astype(str).str.strip().isin(["1", "1.0", "True", "true"])

elements_df = pd.read_csv(
    ELEMENTS_PATH, dtype=str, keep_default_na=False, low_memory=False
)
elements_df.columns = elements_df.columns.str.strip()
for col in elements_df.columns:
    elements_df[col] = elements_df[col].str.strip()
_log("elements.csv read into memory")

_log(f"{len(dpo_df):,} DPO rows, {len(elements_df):,} element rows loaded")
...
_log(f"Index built: {len(dpo_index):,} unique site-key combinations")
_log("Ready")
...
def to_number(value, default=None):
    try:
        ...
        return default
    # NaN survives float() successfully but isn't valid in strict JSON —
    # the browser's response.json() throws a parse error on it. Known
    # materials in particular have no mean_confidence (that field only
    # applies to ML-predicted rows), so this matters a lot for that column.
...
def filter_by_band_gap(min_v: float, max_v: float, statuses=None) -> list:
    """Return every material whose Band_Gap falls inside [min_v, max_v].
    Rows with missing/invalid Band_Gap are ignored. Runs entirely in memory
    against the pre-parsed _band_gap_num column. No result cap — the
    frontend table paginates client-side, so every match is returned.
...
def band_gap_search():
    ...
    materials = filter_by_band_gap(min_v, max_v, statuses)
    return jsonify(
        {
            "materials": materials,
            "total_matches": len(materials),
            "shown": len(materials),
            "min": min_v,
            "max": max_v,
            "statuses": statuses,
